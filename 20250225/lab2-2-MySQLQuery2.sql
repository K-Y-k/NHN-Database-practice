...
-- 외래키 제약조건 설정1 : 참조관계인 기본키 튜플이 삭제될 경우 참조된 릴레이션의 외래키 값으 NULL로 변경해주는 설정
...
-- 외래키 제약조건 설정2 : 참조관계인 기본키 튜플이 삭제될 경우 참조된 릴레이션의 외래키인 튜플도 같이 삭제 설정 cascade
Alter TABLE Product
...
-- 외래키 제약조건 설정3 : 기본키 튜플에 참조된 릴레이션의 외래키인 튜플이 있을 경우 기본키 튜플 삭제되지 않음 (기본값은 ON DELETE NO ACTION)
Alter TABLE Product
ADD CONSTRAINT fk_product_category FOREIGN KEY(CategoryNo)
REFERENCES Category(CategoryNo);
-- ON UPDATE NO ACTION;
